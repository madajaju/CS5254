...
## Learning Objectives
- Analyze the design pressure represented by testing coordination in a stateful concurrent system.
...
## Core Concepts
### Joint Testing
- Definition: Testing the interaction between Object A and Object B, rather than just their individual methods.
...
### Artificial Latency
- Definition: Intentionally slowing down one branch of a parallel workflow to see if the rest of the system handles it correctly.
- Why it matters: This concept identifies a condition that must be made explicit before the system can be trusted under pressure.
- Mechanism: It appears through object state, workflow timing, worker behavior, retries, queues, or coordination boundaries.
- Failure mode: If ignored, the system can pass the happy path while producing duplicate work, stale state, blocked progress, or misleading evidence.
- Design implication: The implementation should expose the relevant invariant, log the critical transition, and validate behavior with a repeatable test.
...
- Definition: Randomly reordering the sequence of incoming "Asset Complete" signals.
- Why it matters: This concept identifies a condition that must be made explicit before the system can be trusted under pressure.
- Mechanism: It appears through object state, workflow timing, worker behavior, retries, queues, or coordination boundaries.
- Failure mode: If ignored, the system can pass the happy path while producing duplicate work, stale state, blocked progress, or misleading evidence.
- Design implication: The implementation should expose the relevant invariant, log the critical transition, and validate behavior with a repeatable test.
...
- Definition: Running 50 workflows at once to find the one timing edge case that leads to a deadlock.
- Why it matters: This concept identifies a condition that must be made explicit before the system can be trusted under pressure.
- Mechanism: It appears through object state, workflow timing, worker behavior, retries, queues, or coordination boundaries.
- Failure mode: If ignored, the system can pass the happy path while producing duplicate work, stale state, blocked progress, or misleading evidence.
- Design implication: The implementation should expose the relevant invariant, log the critical transition, and validate behavior with a repeatable test.
...
Diagram intent:
- Show the normal interaction before Joint Testing is placed under pressure.
...
### Evidence Flow
Claim:
- Testing Coordination is correctly handled when the order workflow preserves prerequisites and progress under pressure.
...
- Identify the invariant or progress property most relevant to testing coordination.
