...
## Learning Objectives
- Analyze the design pressure represented by why locks are not the first answer in a stateful concurrent system.
...
## Opening Narrative
A course project appears correct during a simple demonstration. Once concurrency, ambiguity, or operational pressure is introduced, the design exposes a hidden assumption about Locking as a Patch. What must the engineer analyze and defend before trusting the result?

## Core Concepts
### Locking as a Patch
- Definition: Adding `synchroni
...
### Contention
- Definition: Competition between concurrent actors for a shared object, lock, resource, or signal.
- Why it matters: This concept identifies a condition that must be made explicit before the system can be trusted under pressure.
- Mechanism: It appears through object state, workflow timing, worker behavior, retries, queues, or coordination boundaries.
- Failure mode: If ignored, the system can pass the happy path while producing duplicate work, stale state, blocked progress, or misleading evidence.
- Design implication: The implementation should expose the relevant invariant, log the critical transition, and validate behavior with a repeatable test.
...
Diagram intent:
- Show the normal interaction before Locking as a Patch is placed under pressure.
...
### Evidence Flow
Claim:
- Why Locks Are Not the First Answer is handled when the invariant survives an intentionally bad interleaving.
...
- Identify the invariant or progress property most relevant to why locks are not the first answer.
