...
## Video: https://youtu.be/u9UisXnaLD4
## Metadata
- Course: CS 5254b - Object-Oriented Systems Under Concurrency
- Week: 4
- Lecture: Validating Fixes
- Duration: 15 minutes
...
## Learning Objectives
- Analyze the design pressure represented by validating fixes in a stateful concurrent system.
...
## Opening Narrative
A course project appears correct during a simple demonstration. Once concurrency, ambiguity, or operational pressure is introduced, the design exposes a hidden assumption about Fix Verification. What must the engineer analyze and defend before trusting the result?

## Core Concepts
### Fix Verification
- Definition: Re-running the exact test that previously failed to confirm it now passes reliably.
...
### Boundary Testing
- Definition: Testing the system at its limits (e.g., maximum number of threads) to ensure the fix scales.
- Why it matters: This concept identifies a condition that must be made explicit before the system can be trusted under pressure.
- Mechanism: It appears through object state, workflow timing, worker behavior, retries, queues, or coordination boundaries.
- Failure mode: If ignored, the system can pass the happy path while producing duplicate work, stale state, blocked progress, or misleading evidence.
- Design implication: The implementation should expose the relevant invariant, log the critical transition, and validate behavior with a repeatable test.
...
### Sanity Checks
- Definition: Ensuring that the fix for "Race Condition A" didn't accidentally break "Sequential Feature B."
- Why it matters: This concept identifies a condition that must be made explicit before the system can be trusted under pressure.
- Mechanism: It appears through object state, workflow timing, worker behavior, retries, queues, or coordination boundaries.
- Failure mode: If ignored, the system can pass the happy path while producing duplicate work, stale state, blocked progress, or misleading evidence.
- Design implication: The implementation should expose the relevant invariant, log the critical transition, and validate behavior with a repeatable test.
...
### Proving Correctness
- Definition: Using logs to show that invariants are now preserved even under high pressure.
- Why it matters: This concept identifies a condition that must be made explicit before the system can be trusted under pressure.
- Mechanism: It appears through object state, workflow timing, worker behavior, retries, queues, or coordination boundaries.
- Failure mode: If ignored, the system can pass the happy path while producing duplicate work, stale state, blocked progress, or misleading evidence.
- Design implication: The implementation should expose the relevant invariant, log the critical transition, and validate behavior with a repeatable test.
...
Diagram intent:
- Show the normal interaction before Fix Verification is placed under pressure.
...
### Evidence Flow
Claim:
- Validating Fixes is handled when the invariant survives an intentionally bad interleaving.
...
- Identify the invariant or progress property most relevant to validating fixes.
