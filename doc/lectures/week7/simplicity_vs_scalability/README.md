...
- Week: 7
- Lecture: Simplicity vs. Scalability
- Duration: 15 minutes
...
## Learning Objectives
- Analyze the design pressure represented by simplicity vs. scalability in a stateful concurrent system.
...
## Opening Narrative
A course project appears correct during a simple demonstration. Once concurrency, ambiguity, or operational pressure is introduced, the design exposes a hidden assumption about Cognitive Load. What must the engineer analyze and defend before trusting the result?

## Core Concepts
### Cognitive Load
- Definition: How much effort it takes for a developer to understand the coordination logic.
...
### Resource Saturation
- Definition: When the system's simple design (e.g., one thread per episode) hits the limits of the hardware (e.g., too much memory for thread stacks).
- Why it matters: This concept identifies a condition that must be made explicit before the system can be trusted under pressure.
- Mechanism: It appears through object state, workflow timing, worker behavior, retries, queues, or coordination boundaries.
- Failure mode: If ignored, the system can pass the happy path while producing duplicate work, stale state, blocked progress, or misleading evidence.
- Design implication: The implementation should expose the relevant invariant, log the critical transition, and validate behavior with a repeatable test.
...
- Definition: Can the design handle more work by adding more cores? More machines?
- Why it matters: This concept identifies a condition that must be made explicit before the system can be trusted under pressure.
- Mechanism: It appears through object state, workflow timing, worker behavior, retries, queues, or coordination boundaries.
- Failure mode: If ignored, the system can pass the happy path while producing duplicate work, stale state, blocked progress, or misleading evidence.
- Design implication: The implementation should expose the relevant invariant, log the critical transition, and validate behavior with a repeatable test.
...
- **Podcast Domain**: 
    - *Simple*: A single Java process using `synchroni
...
Diagram intent:
- Show the normal interaction before Cognitive Load is placed under pressure.
...
### Evidence Flow
Claim:
- Simplicity vs. Scalability is correctly handled when the order workflow preserves prerequisites and progress under pressure.
...
- Identify the invariant or progress property most relevant to simplicity vs. scalability.
