...
- Week: 2
- Lecture: The Illusion of Correctness
- Duration: 15 minutes
...
## Learning Objectives
- Analyze the design pressure represented by the illusion of correctness in a stateful concurrent system.
...
## Opening Narrative
A course project appears correct during a simple demonstration. Once concurrency, ambiguity, or operational pressure is introduced, the design exposes a hidden assumption about Sequential Bias. What must the engineer analyze and defend before trusting the result?

## Core Concepts
### Sequential Bias
- Definition: The tendency to design logic assuming Step A *always* finishes before Step B starts.
...
### Implicit Locks
- Definition: In a single-threaded system, the single thread acts as an implicit lock on all state. Removing this "lock" exposes the raw state.
- Why it matters: This concept identifies a condition that must be made explicit before the system can be trusted under pressure.
- Mechanism: It appears through object state, workflow timing, worker behavior, retries, queues, or coordination boundaries.
- Failure mode: If ignored, the system can pass the happy path while producing duplicate work, stale state, blocked progress, or misleading evidence.
- Design implication: The implementation should expose the relevant invariant, log the critical transition, and validate behavior with a repeatable test.
...
### Temporal Coupling
- Definition: When two parts of a system are dependent on each other's timing without explicit coordination.
- Why it matters: This concept identifies a condition that must be made explicit before the system can be trusted under pressure.
- Mechanism: It appears through object state, workflow timing, worker behavior, retries, queues, or coordination boundaries.
- Failure mode: If ignored, the system can pass the happy path while producing duplicate work, stale state, blocked progress, or misleading evidence.
- Design implication: The implementation should expose the relevant invariant, log the critical transition, and validate behavior with a repeatable test.
...
Diagram intent:
- Show the normal interaction before Sequential Bias is placed under pressure.
...
### Evidence Flow
Claim:
- The Illusion of Correctness is handled when the invariant survives an intentionally bad interleaving.
...
- Identify the invariant or progress property most relevant to the illusion of correctness.
