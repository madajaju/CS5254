...
## Learning Objectives
- Analyze the design pressure represented by structured logging & observability in a stateful concurrent system.
...
## Opening Narrative
A course project appears correct during a simple demonstration. Once concurrency, ambiguity, or operational pressure is introduced, the design exposes a hidden assumption about Structured Logging. What must the engineer analyze and defend before trusting the result?
...
- **Anomaly Detection**: Seeing two workers log "Starting audio cleanup" for the same episode ID simultaneously.
...
Diagram intent:
- Show the normal interaction before Structured Logging is placed under pressure.
...
### Evidence Flow
Claim:
- Structured Logging & Observability is defensible when the design shows both the failure and the recovery path with measurements.
...
- Identify the invariant or progress property most relevant to structured logging & observability.
