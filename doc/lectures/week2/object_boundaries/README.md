...
- Week: 2
- Lecture: Object Boundaries & Responsibilities
- Duration: 15 minutes
...
## Learning Objectives
- Analyze the design pressure represented by object boundaries & responsibilities in a stateful concurrent system.
...
## Opening Narrative
A course project appears correct during a simple demonstration. Once concurrency, ambiguity, or operational pressure is introduced, the design exposes a hidden assumption about Encapsulation of State. What must the engineer analyze and defend before trusting the result?

## Core Concepts
### Encapsulation of State
- Definition: Ensuring that an object's internal state can only be modified through its own methods.
...
- Definition: Each object should have one clear purpose (e.g., `Episode` tracks status, `Processor` handles logic).
- Why it matters: This concept identifies a condition that must be made explicit before the system can be trusted under pressure.
- Mechanism: It appears through object state, workflow timing, worker behavior, retries, queues, or coordination boundaries.
- Failure mode: If ignored, the system can pass the happy path while producing duplicate work, stale state, blocked progress, or misleading evidence.
- Design implication: The implementation should expose the relevant invariant, log the critical transition, and validate behavior with a repeatable test.
...
### Leaky Abstractions
- Definition: When an object exposes its internal data structures, allowing external entities to bypass its invariants.
- Why it matters: This concept identifies a condition that must be made explicit before the system can be trusted under pressure.
- Mechanism: It appears through object state, workflow timing, worker behavior, retries, queues, or coordination boundaries.
- Failure mode: If ignored, the system can pass the happy path while producing duplicate work, stale state, blocked progress, or misleading evidence.
- Design implication: The implementation should expose the relevant invariant, log the critical transition, and validate behavior with a repeatable test.
...
Diagram intent:
- Show the normal interaction before Encapsulation of State is placed under pressure.
...
### Evidence Flow
Claim:
- Object Boundaries & Responsibilities is handled when the invariant survives an intentionally bad interleaving.
...
- Identify the invariant or progress property most relevant to object boundaries & responsibilities.
