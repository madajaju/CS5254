...
- Week: 13
- Lecture: Designing for Recovery
- Duration: 15 minutes
...
## Learning Objectives
- Analyze the design pressure represented by designing for recovery in a stateful concurrent system.
...
## Opening Narrative
A course project appears correct during a simple demonstration. Once concurrency, ambiguity, or operational pressure is introduced, the design exposes a hidden assumption about Check-pointing. What must the engineer analyze and defend before trusting the result?

## Core Concepts
### Check-pointing
- Definition: Periodically saving the state of a long-running workflow so it can resume from the last known good point.
...
- Definition: A background process that scans the system for "orphaned" or "stuck" objects and fixes them.
- Why it matters: This concept identifies a condition that must be made explicit before the system can be trusted under pressure.
- Mechanism: It appears through object state, workflow timing, worker behavior, retries, queues, or coordination boundaries.
- Failure mode: If ignored, the system can pass the happy path while producing duplicate work, stale state, blocked progress, or misleading evidence.
- Design implication: The implementation should expose the relevant invariant, log the critical transition, and validate behavior with a repeatable test.
...
### Stateless Workers
- Definition: Designing workers so they don't hold any critical state in memory, allowing them to be restarted without data loss.
- Why it matters: This concept identifies a condition that must be made explicit before the system can be trusted under pressure.
- Mechanism: It appears through object state, workflow timing, worker behavior, retries, queues, or coordination boundaries.
- Failure mode: If ignored, the system can pass the happy path while producing duplicate work, stale state, blocked progress, or misleading evidence.
- Design implication: The implementation should expose the relevant invariant, log the critical transition, and validate behavior with a repeatable test.
...
Diagram intent:
- Show the normal interaction before Check-pointing is placed under pressure.
...
### Evidence Flow
Claim:
- Designing for Recovery is defensible when the design shows both the failure and the recovery path with measurements.
...
- Identify the invariant or progress property most relevant to designing for recovery.
