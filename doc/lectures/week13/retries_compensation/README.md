...
## Learning Objectives
- Analyze the design pressure represented by retries, compensation, dead letter queues in a stateful concurrent system.
...
## Core Concepts
### Exponential Backoff
- Definition: Increasing the wait time between retries to avoid overwhelming a struggling downstream system.
...
- Definition: Taking an action to "undo" a previous successful step because a subsequent step failed (e.g., refunding a payment if shipping fails).
- Why it matters: This concept identifies a condition that must be made explicit before the system can be trusted under pressure.
- Mechanism: It appears through object state, workflow timing, worker behavior, retries, queues, or coordination boundaries.
- Failure mode: If ignored, the system can pass the happy path while producing duplicate work, stale state, blocked progress, or misleading evidence.
- Design implication: The implementation should expose the relevant invariant, log the critical transition, and validate behavior with a repeatable test.
...
- Definition: A separate queue where messages are moved after they have failed all retry attempts, allowing for manual inspection.
- Why it matters: This concept identifies a condition that must be made explicit before the system can be trusted under pressure.
- Mechanism: It appears through object state, workflow timing, worker behavior, retries, queues, or coordination boundaries.
- Failure mode: If ignored, the system can pass the happy path while producing duplicate work, stale state, blocked progress, or misleading evidence.
- Design implication: The implementation should expose the relevant invariant, log the critical transition, and validate behavior with a repeatable test.
...
### Circuit Breaker
- Definition: Temporarily stopping all requests to a failing component to give it time to recover.
- Why it matters: This concept identifies a condition that must be made explicit before the system can be trusted under pressure.
- Mechanism: It appears through object state, workflow timing, worker behavior, retries, queues, or coordination boundaries.
- Failure mode: If ignored, the system can pass the happy path while producing duplicate work, stale state, blocked progress, or misleading evidence.
- Design implication: The implementation should expose the relevant invariant, log the critical transition, and validate behavior with a repeatable test.
...
Diagram intent:
- Show the normal interaction before Exponential Backoff is placed under pressure.
...
### Evidence Flow
Claim:
- Retries, Compensation, Dead Letter Queues is controlled when event handling tolerates duplicate, delayed, or missing delivery evidence.
...
- Identify the invariant or progress property most relevant to retries, compensation, dead letter queues.
