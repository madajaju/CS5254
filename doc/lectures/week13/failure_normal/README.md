...
## Learning Objectives
- Analyze the design pressure represented by failure is normal in a stateful concurrent system.
...
## Core Concepts
### Partial Failure
- Definition: When one component of a system fails while the rest remains operational.
...
### Fail-Fast
- Definition: Detecting an error as soon as possible and stopping the workflow to prevent data corruption.
- Why it matters: This concept identifies a condition that must be made explicit before the system can be trusted under pressure.
- Mechanism: It appears through object state, workflow timing, worker behavior, retries, queues, or coordination boundaries.
- Failure mode: If ignored, the system can pass the happy path while producing duplicate work, stale state, blocked progress, or misleading evidence.
- Design implication: The implementation should expose the relevant invariant, log the critical transition, and validate behavior with a repeatable test.
...
- Definition: Allowing the system to continue functioning with reduced features when a non-essential component fails.
- Why it matters: This concept identifies a condition that must be made explicit before the system can be trusted under pressure.
- Mechanism: It appears through object state, workflow timing, worker behavior, retries, queues, or coordination boundaries.
- Failure mode: If ignored, the system can pass the happy path while producing duplicate work, stale state, blocked progress, or misleading evidence.
- Design implication: The implementation should expose the relevant invariant, log the critical transition, and validate behavior with a repeatable test.
...
### Blast Radius
- Definition: Limiting the impact of a failure so it doesn't bring down the entire system.
- Why it matters: This concept identifies a condition that must be made explicit before the system can be trusted under pressure.
- Mechanism: It appears through object state, workflow timing, worker behavior, retries, queues, or coordination boundaries.
- Failure mode: If ignored, the system can pass the happy path while producing duplicate work, stale state, blocked progress, or misleading evidence.
- Design implication: The implementation should expose the relevant invariant, log the critical transition, and validate behavior with a repeatable test.
...
Diagram intent:
- Show the normal interaction before Partial Failure is placed under pressure.
...
### Evidence Flow
Claim:
- Failure is Normal is defensible when the design shows both the failure and the recovery path with measurements.
...
- Identify the invariant or progress property most relevant to failure is normal.
