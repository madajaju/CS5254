...
## Learning Objectives
- Analyze the design pressure represented by duplicate execution in a stateful concurrent system.
...
## Opening Narrative
A worker finishes a job, but its acknowledgement is lost before the queue records success. The same message is delivered again, and a second worker repeats the side effect. The system is not broken because the queue retried; it is broken because the operation was not designed to tolerate duplicate execution. What must change so repeated delivery does not corrupt the workflow?

## Core Concepts
### At-Least-Once Delivery
- Definition: A common guarantee where a message is guaranteed to be delivered but might be delivered more than once.
...
### Double Processing
- Definition: When two different workers pick up the same task, or one worker picks it up twice.
- Why it matters: This concept identifies a condition that must be made explicit before the system can be trusted under pressure.
- Mechanism: It appears through object state, workflow timing, worker behavior, retries, queues, or coordination boundaries.
- Failure mode: If ignored, the system can pass the happy path while producing duplicate work, stale state, blocked progress, or misleading evidence.
- Design implication: The implementation should expose the relevant invariant, log the critical transition, and validate behavior with a repeatable test.
...
- Definition: The negative impact of doing the same work twice (e.g., charging a customer twice, sending two notifications).
- Why it matters: This concept identifies a condition that must be made explicit before the system can be trusted under pressure.
- Mechanism: It appears through object state, workflow timing, worker behavior, retries, queues, or coordination boundaries.
- Failure mode: If ignored, the system can pass the happy path while producing duplicate work, stale state, blocked progress, or misleading evidence.
- Design implication: The implementation should expose the relevant invariant, log the critical transition, and validate behavior with a repeatable test.
...
Diagram intent:
- Show the normal interaction before At-Least-Once Delivery is placed under pressure.
...
### Evidence Flow
Claim:
- Duplicate Execution is controlled when event handling tolerates duplicate, delayed, or missing delivery evidence.
...
- Identify the invariant or progress property most relevant to duplicate execution.
